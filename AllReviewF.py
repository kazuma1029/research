...
if choice=="2":#アンダーサンプリングを行う場合
    choiceA=input("アンダーサンプリングのモードを選択してください（1:スコアが高いレビューを残す, 2: ランダム)： ")
    if choiceA=="1":#スコアが高いレビューを残すアンダーサンプリングを行う場合
        ...
        print(f"liked_scores:{len(liked_scores)}")
        print(f"disliked_scores:{len(disliked_scores)}")
        if abs(len(liked_scores) - len(disliked_scores)) !=0:
            if len(liked_scores) > len(disliked_scores):
                ...
                liked_reviews = [r[0] for r in liked_scores]
                liked_labels=liked_labels[:len(liked_reviews)]
            else:
                disliked_scores.sort(key=lambda x: x[1],reverse=True)
                disliked_scores = disliked_scores[:len(liked_scores)]
                disliked_reviews = [r[0] for r in disliked_scores]
                disliked_labels=disliked_labels[:len(disliked_reviews)]
    
    elif choiceA=="2":
        # 好みの映画と好みでない映画のレビュー数をランダムに揃える
...
print(f"被験者{hikensya_num}の好みの映画のレビュー数は{len(liked_reviews)}です．")
print(f"被験者{hikensya_num}の好みでない映画のレビュー数は{len(disliked_reviews)}です．")
...
# ステップ12: モデルを保存
if choice=="1":
    trainer.save_model(f"./models/{hikensya_num}_allmodel")
    tokenizer.save_pretrained(f"./models/{hikensya_num}_allmodel")
if choice=="2":
    if choiceA=="1":
        trainer.save_model(f"./models/{hikensya_num}_allmodel_us")
        tokenizer.save_pretrained(f"./models/{hikensya_num}_allmodel_us")
