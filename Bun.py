...
def save_results_to_excel(hikensya_num, metrics, choice,choiceA,choiceB, n):
    # ファイルパス
    ...
    # 書き込む列の選択
    if choice == "1":
        if choiceA == "1": #　全レビューかつアンダーサンプリングなし
            col = 3  # 3列目に記録
 
        elif choiceA=="2":#全レビューかつアンダーサンプリングあり
            if choiceB=="1":#スコアアンダーサンプリング
                col = 4  #4列目に記録
            elif choiceB=="2":#ランダムアンダーサンプリング
                col = 5 #5列目に記録
        # 適合率（3～8行目)
        ...
        if n is not None and n >= 100:
            base_row = 3 + ((n // 100) - 1)  # 100, 200, 300...に対応

            sheet.cell(row=base_row, column=col).value = precision    # 適合率
            sheet.cell(row=base_row + 10, column=col).value = recall  # 再現率
            sheet.cell(row=base_row + 20, column=col).value = f1      # F値
...
def main(hikensya_num):
    choice = input("分類モードを選択してください（1: 全レビューのモデル, 2: 抽出レビューのモデル）: ")
    choiceA = None  # 初期値を設定
    choiceB = None  # 初期値を設定
    if choice=="1":
        choiceA= input("アンダーサンプリングの有無を選択してください（1: なし, 2: あり ）: ")
        if choiceA=="2":
                choiceB=input("アンダーサンプリングのモードを選択してください（1:スコアが多いレビューを残す, 2: ランダム： ")


    if choice == "1":
        if choiceA=="1":
            model_path = f"./models/{hikensya_num}_allmodel"
            print(f"全レビューでファインチューニングしたモデルを使用します: {model_path}")
            model, tokenizer = load_finetuned_model(model_path)

        if choiceA=="2":
            if choiceB=="1":
                model_path = f"./models/{hikensya_num}_allmodel_us"
                print(f"全レビューでファインチューニングし，スコアに基づきアンダーサンプリングしたモデルを使用します: {model_path}")
                model, tokenizer = load_finetuned_model(model_path)
            elif choiceB=="2":
                model_path = f"./models/{hikensya_num}_allmodel_rus"
                print(f"全レビューでファインチューニングし，ランダムにアンダーサンプリングしたモデルを使用します: {model_path}")
                model, tokenizer = load_finetuned_model(model_path)
        ...
        # 結果をExcelに保存
        save_results_to_excel(hikensya_num, metrics, choice,choiceA,choiceB,n=None)

    elif choice == "2":
        n_values = list(range(100, 1001, 100))

        for n in n_values:
            model_path = f"./models/nounmodels/{hikensya_num}/{n}"
            if not os.path.exists(model_path):
                print(f"モデルが存在しません: {model_path}")
                break
            print(f"n={n}のモデルを使用します．: {model_path}")
            ...
            metrics = compute_metrics(all_predictions, all_labels)
            print(f"n={n} のモデル分類結果:", metrics)

            # 結果をExcelに保存
            save_results_to_excel(hikensya_num, metrics, choice,choiceA,choiceB, n)
